...
int main(int argc, char ** argv) {
    ...
    /// Store mui::point2d that located in the fetch interface
 	for ( int k = 0; k < Nt2; ++k ) {
                    mui::point3d ptf( points2[k][0], points2[k][1], points2[k][2]);
    ...
    outputFile << "\"X\",\"Y\",\"Z\",\"scalar_field\"\n";
 	for ( int k = 0; k < Nt2; ++k ) {
                outputFile << points2[k][0] << "," <<points2[k][1]<< "," << points2[k][2]<< "," << scalar_field[k] << ", \n";
    }
    outputFile.close();
    ...
    for ( int t = 1; t <= steps; ++t ) {
        ...
        for ( int k = 0; k < Nt2; ++k ) {
			...
			mui::point3d locf( points2[k][0], points2[k][1], points2[k][2]);

			/// Fetch data from the other solver
			scalar_field[k] = ifs[0]->fetch(  name_fetch,
													locf, 
													t, 
													spatial_sampler, 
													chrono_sampler);
			...
			ifs[1]->push( name_push, locp, scalar_field[k] );
